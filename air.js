...
$(function(){
  var windowWidth, width, marginTop, height, canvas, svg, minLatitude, maxLatitude, minLongitude, maxLongitude, dy, dx, proj, drawTaiwan, ConvertDMSToDD, drawStations, currentMetric, currentUnit, colorOf, stations, setMetric, drawSegment, addList, epaData, samples, distanceSquare, idwInterpolate, yPixel, plotInterpolatedData, updateSevenSegment, drawHeatmap, drawAll;
  ...
  setMetric = function(name){
    var ref$, y, xOff, yOff, i$, len$, c, legend;
    currentMetric = name;
    ...
    for (i$ = 0, len$ = (ref$ = colorOf.domain()).length; i$ < len$; ++i$) {
      ...
      legend.append('text').attr('x', 55 + xOff).attr('y', y + 15 + yOff).attr('d', '.35em').text(c + currentUnit).style('fill', '#AAAAAA').style('font-size', '10px');
    }
